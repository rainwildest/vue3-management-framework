...
const storage = {
   ...
   * @param {*} expired 到期时间
   */
  set(key: unknown, value: unknown, expired?: number) {
    if (!key) throw "storage 执行 set 方法，需要参数 key。";

    if (isNull(value)) throw "storage 执行 set 方法，需要参数 value。";
   ...
   */
  get(key: string) {
    if (!key) throw "storage 执行 get 方法，需要参数 key。";

    const source: any = { key, value: null };
    const now = Date.now();
